            ...
            Column(
                ...
                for (franjaHoraria in obtenerHorario()) {
                    Row(modifier = Modifier.fillMaxWidth()) {
                        Text(
                            text = franjaHoraria.first,
                            style = MaterialTheme.typography.bodyMedium
                        )
                        Spacer(modifier = Modifier.weight(1f))
                        Text(
                            text = franjaHoraria.second,
                            style = MaterialTheme.typography.bodyMedium
                        )
                    }
                }
    ...
    private fun obtenerHorario(): List<Pair<String, String>> {
        ...
        return dias.flatMap { dia ->
            listOf(
                ...
                Pair(
                    "$dia ${7 + duracionClase}:00 AM",
                    "$dia ${7 + 2 * duracionClase}:00 AM"
                ), // Lunes 9:00 AM - 11:00 AM
                Pair(
                    "$dia ${7 + 2 * duracionClase}:00 PM",
                    "$dia ${7 + 3 * duracionClase}:00 PM"
                ), // Lunes 2:00 PM - 4:00 PM
                Pair(
                    "$dia ${7 + 3 * duracionClase}:00 PM",
                    "$dia ${7 + 4 * duracionClase}:00 PM"
